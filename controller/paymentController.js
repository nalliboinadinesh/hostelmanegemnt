...
module.exports = { getPaymentsByTenant, getPaymentsByHostel, updatePayment, getFeeDataByHostel };

// GET /api/payment/fee/:hostelId
// Full fee data: each tenant with personal info, room details, feeStatus, and all payment cycles
const getFeeDataByHostel = async (req, res) => {
  try {
    const hostel = await verifyHostelOwner(req.params.hostelId, req.owner._id);
    if (!hostel) {
      return res.status(404).json({ message: 'Hostel not found or unauthorized' });
    ...
    // Fetch all tenants with floor + room populated
    const tenants = await Tenant.find({ hostelId: req.params.hostelId })
      .populate('floorId', 'floorNumber')
      .populate('roomId', 'roomNumber roomType totalBeds occupiedBeds vacantBeds')
      .lean();

    if (!tenants.length) {
      return res.status(200).json({ hostelId: req.params.hostelId, total: 0, tenants: [] });
    }

    // Fetch all payments for all tenants in one query
    const tenantIds = tenants.map(t => t._id);
    ...
    const result = tenants.map(tenant => {
      ...
      const lastPaid     = payments.filter(p => p.isPaid).pop() || null;
      ...
      return {
        ...
        parentNumber:  tenant.parentNumber || null,
        ...
        // Room details
        floor: tenant.floorId
          ? { floorId: tenant.floorId._id, floorNumber: tenant.floorId.floorNumber }
          : null,
        room: tenant.roomId
          ? {
              ...
              totalBeds:    tenant.roomId.totalBeds,
              occupiedBeds: tenant.roomId.occupiedBeds,
              vacantBeds:   tenant.roomId.vacantBeds,
            }
          : null,

        // Fee status (per calendar month)
        ...
        paymentSummary: {
          ...
          lastPaidMethod: lastPaid?.paymentMethod || null,
        },
    ...
    // Hostel-level summary
    const summary = {
      totalTenants:    result.length,
      paidTenants:     result.filter(t => t.paymentStatus === 'paid').length,
      pendingTenants:  result.filter(t => t.paymentStatus === 'pending').length,
      totalDues:       result.reduce((sum, t) => sum + t.paymentSummary.totalDues, 0),
      totalCollected:  result.reduce((sum, t) => sum + t.paymentSummary.totalPaidAmount, 0),
    };
